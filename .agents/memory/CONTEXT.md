...
- **已关闭：准入规则的覆盖缺口与一处真缺陷（2026-09-26）**。审计口径过期一半：`runHandoff` 已不存在、`statusText` 早有直接断言；真正缺的是「非法名字 / 候选无归档证据 / 候选已存在」三个分支（已通过 pass 路径钉住，`9bd452c`，三个有效变异体），以及**第三遍核对才抓到的第四个**：`shapeRejection` 的 **`description too long` 全库 0 钉子**。顺着它探针复现，发现这不是「没测试」而是**规则在 approve 路径上不可达**——`approveCandidate` 用会**截断**到 1024 的 `skillDescription()` 重读候选文件，于是手工 approve 能激活一份 pass 会拒绝的超限文档（实测 5000 字描述 → `{ok:true}` + 静默截断）。修法：校验读未截断的前插值，只有落盘值截断（`a627eb5`，两个变异体各掉 1）；该规则补进三角度案例表后，`shapeRejection` + `rejectionReason` 的**九个**返回串全部有钉。**教训：这条是我上一批宣称「准入规则已全部钉住」时漏掉的——靠逐个 grep 每个 `return` 串的机器核对才暴露，不能靠读注释。**

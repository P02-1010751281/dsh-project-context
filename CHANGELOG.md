...
- 变更：`resolveThreshold` **拆开**为单一职责的纯函数——`handoffRoom`（① 只判可行性）、
  ...
  两者只能因**组合**而分歧。
- 修复：**未答问题的延后吃掉了 15 秒的测压节流窗口**，于是用户回答之后的**第一个** `turn/end` 被丢弃。
  节流的作用是限制 `resolveModelInfo` + `tokenMeter.measure` 的频率，而**延后**必须释放它——延后的条件
  正是下一次空闲要解决的事：`HandoffDeferred`（监听器的 catch）与「子代理仍在跑」两支都显式删除了时间戳，
  只有未答问题这一支没删。默认 `handoffPendingQuestion` 就是 `defer`，所以这是默认路径；实测（生产 15s
  间隔、200K 窗口、压力 190K）回答后的下一次调用被节流丢弃，交接要等满一个窗口才可能发生，而这段时间会话
  还在继续增长。现在该分支与另外两支一致地释放节流，回答后的第一个空闲立刻重新测压并完成交接。
  另两个提前返回**不是**延后、继续消耗窗口：窗口解析不出来与压力未到阈值都属于「无事可做」，节流正是为
  这两者存在的。变异校验：去掉该释放，新测试掉 1 项。

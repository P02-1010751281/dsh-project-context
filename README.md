...
- ④：摘要输入是 `MEMORY.md`、最近对话窗口与文件操作索引，不依赖整理是否运行；摘要失败退避 5 分钟；未答问题按 `handoffPendingQuestion` 处理——`wait` 会把问题原文作为独立段落带进新会话（此前只跳过延后、问题实际会丢），并以该段落**取代**常规的“先做下一步”结尾。自动交接在**本会话还有未结束的后台子代理时延后**（优先读 `subagents` 服务的实时子会话列表，取 `mode: continuable` 且 `activity: running` 的；该服务缺失或查询失败时回退到会话日志扫描）：子代理落定会唤醒父会话并开启新一轮，此时交接会变成父子两个会话同时改同一个项目（本仓库 2026-09-16 实际踩到过），所以等到没有在跑的子代理再交；**同一个失败还会从另一侧发生**——触发点是 `turn/end`，但 harness 会在当前轮一关就立刻把队列里的用户消息开成下一轮，所以只要交接时队列里已经有消息，父会话在插件写摘要的那几秒里就已经在跑新一轮了（本仓库 2026-09-17 实际踩到过：`turn/end` 与下一轮 `turn/start` 只差 4 毫秒，8.3 秒后子会话被创建，父子同时改了同一处代码）。所以自动交接在**触发它的那个 `turn/end` 之后本会话又开了新轮**时直接放弃（不算失败、不占失败退避、不写 `errors.log`，只记一条限频服务端日志）。检查点有三处：摘要调用前、摘要调用后、以及**投递首条消息前**（创建子会话是 RPC、再带上权限与模型，这段窗口里开的新轮同样要拦）——前两处没创建任何东西，第三处子会话已存在，于是不播种、并把标题改掉（不带切换标记，浏览器不会切过去），只留一个空会话；下一次尝试照常。判定读的是会话日志里更晚的 `turn/start`，因为 `turn/end` 提交时 driver 的 phase 仍是 `running`，读 `agent.status` 无法区分“这一轮刚结束”和“已经在跑下一轮”；落在某次尝试进行中的 `turn/end` 不会被丢掉，而是记下来、等这次尝试结束后**按它补评**（否则一次被吞掉的、真正空闲的 `turn/end` 可能让该会话再也不交接；若这次尝试以失败告终，则补评先受 5 分钟失败退避约束，不会立刻重试）。手动 `/handoff now` 不受这条限制（它本身就在一轮里执行）。**自动**交接因“没有更早内容可摘要”而跳过时只写一条**限频的服务端日志**（约 10 分钟一次）——dsh 目前没有 host 侧的通知服务，UI 里不会弹提示；要看当时的上下文占用用 `/handoff status`，要强制移交用 `/handoff now`（手动路径的回执是可见的）。没有可摘要的更早消息时（空会话，或整段对话都落在 `handoffKeepTokens` 原文窗口内）直接拒绝，手动 `/handoff now` 回一条错误说明而不是伪造摘要，并提示用 `/handoff keep 0` 摘要整段对话。交接**跟随对话语言**（`handoffLanguage`，含摘要指令、六个段落标题与首条消息）；重放中**上一轮交接提示会被替换成一行标记**，不再把陈旧的交接提示原样带进孙会话。带入的"最近对话原文"是把消息渲染成文本（`## user` / `## assistant` / `## tool result`，助手只调工具时是 `[tool: 名称]`）——dsh 的交接只能把文本投给子会话、不能重放结构化消息，所以这里是**渲染**而不是 pi 的**真消息重放**；**工具输出会一并带入**（每条截断 1500 字符，`textOf` 读 `tool-result` 块），同一份渲染也是整理/沉淀的输入。新会话沿用父会话的 **agent preset**、**权限预设**与**当前模型/思考级别**（`sessionController.create` 既不接受 model 也不接受权限：创建后先按 `permissionPresets.current(父会话)` 取预设并经服务的 `set(子会话, 预设)` 写入沙箱模式与审批策略，再按父会话最后一次请求的路由调 `selectModel`，最后才投递首条消息——否则用户切到的"完全访问"到了子会话会退回"每个敏感操作都要问"；父会话是 `custom` 组合、子会话尚未进入内存、或运行时没有这些服务时按原样创建，失败只记警告不影响交接），workspace 按 cwd **精确匹配**接入（匹配不到就只用 cwd 创建），首条消息里的存档指针是绝对路径、`HANDOFF.md` 里写仓库相对路径；浏览器端在标题出现时自动切换，但**当前会话输入框非空或正在提交时不抢**。阈值 0.4 早于 dsh 内置压缩的 0.8，两者可共存。

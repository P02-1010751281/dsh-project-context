...
test("the quality knee is a fallback cap: it never overrides the configured target and never clamps silently", async () => {
	// pi's fitted MRCR curve is the only obtainable *quality* signal — no provider API exposes one —
	// and its purpose is to distrust a declared window (models advertising 1M measure 130–170K). dsh
	// applies it as a cap on an already-decided value, upstream/config first; pi instead takes
	// `max(knee, target)`, where the config target *raises* the threshold above the curve. So the port
	// is a safety net, not a behaviour change: with the defaults the curve is above the configured
	// value at every window, and it only bites once `/handoff target` is raised past it.
	const signal = new AbortController().signal;
	const measurement = { totalTokens: 26_000, surfaceTokens: 20_000 }; // baseline 6000
	const session = {
		id: "session-knee-000000000000",
		header: { cwd: process.cwd(), createdAt: Date.now() },
		deriveMessages: () => [],
		requestHeader: () => undefined,
		snapshotEvents: () => [],
	};
	const ctxAt = (contextWindow) => ({
	...
	// ① Safety net. Recompute the pre-knee formula independently instead of trusting the constants the
	// implementation happens to carry: the threshold must be value-for-value what it was before.
	for (const window of [65_536, 200_000, 400_000, 1_000_000, 2_000_000]) {
		const usable = window - 16_384;
		const room = usable - 6_000 - defaults.handoffKeepTokens;
		const older = Math.max(8_000, Math.min(defaults.handoffTargetTokens, Math.floor(room / 2)));
		const before = Math.min(6_000 + defaults.handoffKeepTokens + older, usable - 4_000);
		const threshold = resolveThreshold(defaults, measurement, window);
		assert.deepEqual(
			[threshold.tokens, threshold.bound],
			[before, "target"],
			`the knee changed the default threshold at ${window}`,
		);
	}
	// ② Raised target. The curve caps it at its asymptote on a window the declaration overstates, and
	// the bound records which term decided.
	const raised = config({ handoffTargetTokens: 200_000 });
	assert.equal(resolveThreshold(raised, measurement, 1_000_000).tokens, 157_000);
	assert.equal(resolveThreshold(raised, measurement, 1_000_000).bound, "knee");
	assert.equal(resolveThreshold(raised, measurement, 2_000_000).bound, "knee");
	// Where the curve is still above the target the target wins: this is a fallback, not a replacement.
	// `conversationRoom / 2` caps the target at 178_808 at this window, so the configured value is
	// 204_808 — well below the curve's 387_852, which therefore does not bind.
	assert.equal(resolveThreshold(raised, measurement, 400_000).tokens, 204_808);
	assert.equal(resolveThreshold(raised, measurement, 400_000).bound, "target");
	// ③ Fixed mode returns before the curve, so an explicit user ratio is never silently overridden.
	const fixed = config({ handoffAdaptive: false, handoffThresholdRatio: 0.5 });
	assert.equal(resolveThreshold(fixed, measurement, 1_000_000).tokens, 500_000);
	assert.equal(resolveThreshold(fixed, measurement, 1_000_000).bound, undefined, "the curve is an auto-mode fallback only");
	// ④ The clamp is not silent. Lowering a value the user configured without saying so is the failure
	// mode the fence decision rules out, so the receipt names the curve when the curve decided.
	const clamped = await statusText(ctxAt(1_000_000), session, raised, signal);
	assert.match(clamped, /capped by the model's quality knee/, `a knee clamp must be named: ${clamped}`);
	const decided = await statusText(ctxAt(400_000), session, raised, signal);
	assert.doesNotMatch(decided, /quality knee/, "no suffix when the target is what decided");
	// ⑤ The curve is the only term that can fall below the floor once the window clears it, so the
	// refusal companion has a cause for it rather than blaming the safety margin.
	const heavy = { totalTokens: 260_000, surfaceTokens: 20_000 }; // baseline 240_000
	assert.equal(resolveThreshold(defaults, heavy, 2_000_000), undefined, "a floor above the curve really refuses");
	assert.equal(thresholdRefusal(defaults, heavy, 2_000_000), "knee-below-floor");
	assert.match(thresholdRefusalText("knee-below-floor", defaults, heavy, 2_000_000), /quality knee is the limit/);
});
